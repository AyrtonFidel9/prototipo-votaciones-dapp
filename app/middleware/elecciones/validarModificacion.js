  ...
  if (eleccion.estado === "NO-INICIADO") {
    ...
    if (estado !== "EN-CURSO")
      return res.status(400).send({
        message: "La elección solo puede cambiar al estado de EN CURSO",
      });

    if (horaActual < horaInicial && estado === "EN-CURSO")
      return res.status(400).send({
        message:
          "La elección no puede ser modificada al estado de EN CURSO, por la fecha",
      });

    if (horaActual > horaFinal && estado === "EN-CURSO")
      return res.status(400).send({
        message:
          "La elección no puede ser modificada al estado de EN CURSO, por la fecha",
      });
    ...
    if (horaActual >= horaInicial && horaActual <= horaFinal)
      return res.status(400).send({
        message: "La elección esta EN CURSO, no puede ser modificada ",
      });

    if (horaActual > horaInicial && estado !== "NO-INICIADO") {
      if (estado !== "EXITOSO") {
        return res.status(400).send({
          message: "La elección solo puede cambiar al estado de NO-INICIADO",
        });
      }
    } else if (horaActual < horaFinal && estado !== "EXITOSO") {
      return res.status(400).send({
        message: "La elección solo puede cambiar al estado de EXITOSO",
      });
    }
  } else if (eleccion.estado === "EXITOSO") {
    if (estado !== "NULIDAD" && estado !== "IMPUGNADO")
      return res.status(400).send({
        message:
    ...
    if (fecEle !== fecHoy && estado === "EN-CURSO")
      return res.status(400).send({
        message: "La elección no puede ser modificada al estado de EN CURSO",
      });
  }
