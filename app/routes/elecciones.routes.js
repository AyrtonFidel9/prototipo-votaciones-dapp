...
routerElecciones
  .route("/:idEleccion")
  .get([authJwt.verifyToken, authJwt.isJGE], (req, res) => {
    EleccionesController.getEleccion(req, res);
  });

routerElecciones.route("/").get([authJwt.verifyToken], (req, res) => {
  EleccionesController.getAllElecciones(req, res);
});

routerElecciones
  .route("/delete/:idEleccion")
  .delete(
    [authJwt.verifyToken, authJwt.isJGE, validarEliminacion],
    (req, res) => {
...
routerElecciones
  ...
  .put(
    ...
    (req, res) => {
      return EleccionesController.updateEleccion(req, res);
    }
  );
