...
export const justificacionFindOne = async (id) => {
  try {
    ...
    if (justificacion === null)
      throw `No existe una justificacion con el id: ${id}`;
    else
      return {
        status: 200,
        message: justificacion,
...
// Find a single Incripciones with an id
export const justificacionFindEleccionaAndSocio = async (
  idSocio,
  idEleccion
) => {
  try {
    const justificacion = await Justificacion.findOne({
      where: {
        idSocio: idSocio,
        idElecciones: idEleccion,
      },
    });
    if (justificacion === null) return true;
    else return false;
  } catch (e) {
    throw {
      status: 400,
